...
def get_distance_mappings(model):
    import inspect
    exec("from moose_nerp.{}.param_cond import *".format(model))
    {key:value for key,value in inspect.currentframe().f_locals if isinstance(value, tuple) and len(value) in [2,3]} # Not needed to find distance mapping function
...
def get_condutace_block_start(new_param_cond, neuron_type):
    c_line_pattern = '^_{}\s*=\s*_util.NamedDict\(.*$'.format(neuron_type)
    ...
    with fileinput.input(files=(new_param_cond), inplace=True) as f_obj:
        for line in f_obj:
            if start_block_lineno < f_obj.lineno() < end_block_lineno:
                if re.search("(?P<chan>\w+)={([\sa-zA-Z0-9\.:,]+)", line):
                    chan = re.search("(?P<chan>\w+)={([\sa-zA-Z0-9\.:,]+)", line).group('chan')
                    chunks = re.split("([\sa-zA-Z0-9\.:,]+)", line)
                    logger.debug('{}'.format(chunks))
...
def get_modified_sub_string(sub_str, chan, conds):
    if re.match("[\sa-z-Z0-9\.:,]+", sub_str) and ':' in sub_str and ',' in sub_str:
...
def create_npz_param(npz_file, model, neuron_type, store_param_path=None,
    ...
    #TODO: Get import dist and proc and key values pairs. Not needed
